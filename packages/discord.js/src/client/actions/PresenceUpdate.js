...
class PresenceUpdateAction extends Action {
  handle(data) {
    ...
    let presences = guild ? guild.presences : this.client.relationships.presences;
    const oldPresence = presences.cache.get(user.id)?._clone() ?? null;
    if (guild) {
    ...
    const newPresence = presences._add(Object.assign(data, { guild }));
    if (this.client.listenerCount(Events.PresenceUpdate) && !newPresence.equals(oldPresence)) {
      /**
       * Emitted whenever a guild member's or a user's  presence (e.g. status, activity) is changed.
